...
def validacija_leta(broj_leta: str, sifra_polazisnog_aerodroma: str, sifra_odredisnog_aerodroma: str,
    ...
    try:
        ...
        if int(sletanje_minuti) >= 60 or int(poletanje_minuti) >= 60:
            raise izuzeci.NeispravnoVreme(f"Greška - Minuti ne mogu biti veći od 59. ({poletanje_minuti} | {sletanje_minuti})")
        if int(sletanje_sati) >= 24 or int(poletanje_sati) >= 24:
            raise izuzeci.NeispravnoVreme(f"Greška - Sati ne mogu biti veći od 23. ({poletanje_sati} | {sletanje_sati})")
    except:
        raise izuzeci.NeispravnoVreme(f"Greška - Vreme se unosi u formati hh:mm. ({vreme_poletanja} | {vreme_sletanja})")

    if type(sletanje_sutra) != bool:
        raise izuzeci.NeispravnoUnetiPodaci(f"Greška - sletanje_sutra mora biti True ili False. ({sletanje_sutra})")

    if cena < 0:
        raise izuzeci.NeispravnoUnetiPodaci(f"Greška - cena mora biti broj veći od 0. ({cena})")
...
def checkin(svi_letovi: dict, konkretan_let: dict, red: int, pozicija: list) -> list:
    ...
    ukupan_br_redova = svi_letovi[konkretan_let['broj_leta']]['model']['broj_redova']
    ...
    for konkretna_pozicija in pozicija:
        if konkretna_pozicija not in pozicije_sedista:
            raise izuzeci.NeispravnoUnetiPodaci(f"Greška - Pozicija ne postoji! ({konkretna_pozicija} -> {pozicije_sedista})")

    i = 0
    while i < len(pozicije_sedista):
        for konkretna_pozicija in pozicija:
    ...
    for konkretan_let in konkretni_letovi:
        ...
        if datum_polaska == konkretni_letovi[konkretan_let]['datum_i_vreme_polaska'] or datum_polaska == "":
            if datum_dolaska == konkretni_letovi[konkretan_let]['datum_i_vreme_dolaska'] or datum_dolaska == "":
                if let['sifra_odredisnog_aerodroma'] == odrediste or odrediste == "":
...
def trazenje_10_najjeftinijih_letova(svi_letovi: dict, polaziste: str = "", odrediste: str = ""):
    ...
    for let in svi_letovi:
        if polaziste == svi_letovi[let]['sifra_polazisnog_aerodroma'] or polaziste == "":
            if odrediste == svi_letovi[let]['sifra_odredisnog_aerodroma'] or odrediste == "":
...
def kreiranje_letova(svi_letovi: dict, broj_leta: str, sifra_polazisnog_aerodroma: str, sifra_odredisnog_aerodroma: str,
                     vreme_poletanja: str, vreme_sletanja: str, sletanje_sutra: bool, prevoznik: str,
                     dani: list, model: dict, cena: int) -> dict:
    validacija_leta(broj_leta, sifra_polazisnog_aerodroma, sifra_odredisnog_aerodroma,
                    vreme_poletanja, vreme_sletanja, sletanje_sutra, prevoznik, dani, model, cena)
    svi_letovi.update(podesi_let(broj_leta, sifra_polazisnog_aerodroma, sifra_odredisnog_aerodroma,
                      vreme_poletanja, vreme_sletanja, sletanje_sutra, prevoznik, dani, model, cena))
    return svi_letovi
